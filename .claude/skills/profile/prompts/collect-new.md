...
2. 在最多 5 次 web_search 和最多 3 次 web_fetch 的预算内，设计一组**覆盖尽可能多目标信息**的查询和抓取计划。
   - 每个 web_search 的 query 可以同时涵盖多个目标（例如"官网+工商+融资+招聘"等），不要为每个信息点单独开一个 query。
   - 官网相关页面（首页/产品/关于）优先通过 1–2 次 web_fetch 获取。
   - 预留 1–2 次 web_search 专门用于 `mainCompetitors` / `knownClients` / `targetCustomers` 这类需要独立查询的字段。

当你准备好后，再一次性发起一批工具调用；在同一轮中，你可以同时抛出多个 web_search 和 web_fetch 调用，由外部并行执行。

---

## 第一部分：信息采集（工具预算：最多 5 次 web_search + 3 次 web_fetch）
...
你需要自行规划如何在这 8 次调用内覆盖尽量多的以下信息点。竞品、已知客户、目标客户这三类字段通常需要独立的专项查询，请留出预算。
